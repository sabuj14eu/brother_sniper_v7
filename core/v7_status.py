...
def _push(record: dict, path: str) -> None:
    ...
    # [2026-09-03 port to the deploy branch] this module was born on the
    # trade-desk branch with PLATFORM_URL/PLATFORM_SECRET; every other poster
    # on this branch reads PLATFORM_WEBHOOK_URL/PLATFORM_WEBHOOK_SECRET. Accept
    # either pair (append-only), so a box .env written for one branch works.
    base = (os.getenv("PLATFORM_URL") or os.getenv("PLATFORM_WEBHOOK_URL") or "").rstrip("/")
    secret = os.getenv("PLATFORM_SECRET") or os.getenv("PLATFORM_WEBHOOK_SECRET") or ""
