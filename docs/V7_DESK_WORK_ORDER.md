...
| 6 | Setup Edge (side×grade×setup×session×symbol) | **BUILT 08-19** | `setup_edge.py` → `evidence.json["setup_edge"]`. 8 families (session×side, symbol×side, symbol×session, grade×side, setup×side, setup×session, regime×side, symbol×side×session). Each family reports `coverage_pct` (fields never recorded ⇒ NOT MEASURED, not "no edge"), `hidden_thin` (buckets below n=3, counted never dropped) and `truncated`. Verdicts reuse gate_effectiveness thresholds; below n=20 everything reads UNPROVEN. At 194 trades the 3-way cut is UNPROVEN throughout — expected, and shown as such |
